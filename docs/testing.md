# тестить нужно против продуктовой базы или по крайней мере двойника
нужен repository layer
выводы    
[https://learn.microsoft.com/en-us/ef/core/testing/choosing-a-testing-strategy#summary]

- Лучше SQL Server Developer edition  чем LocalDB, since it provides the full SQL Server feature set and is generally very easy to do.
SQL azure почти не чем не отличается от локальной SQL server
 Azure Cosmos DB используй, the Azure Cosmos DB emulator
...
## add tag длч удобства поиска SQL в логе
...
#  используй для мониторинга на стороне сервера
